...
result_dir='/u/stagnerl/FIDASIM/RESULTS/D3D/'
                   ;; Location where results will be stored /RESULTS/runid will be made
profile_dir='/u/heidbrin/OANB/AUG/'
                   ;; Location of profile save files. EX: profile_dir+'/dne142353.00505'

;;----------------------------------------------------
;; Fast-ion distribution function from transp
;;----------------------------------------------------
cdf_file='/e/alfven/FIDAsim/D3D/146088/146088H02_fi_9.cdf'
                   ;; CDF file from transp with the distribution function
...
dlambda= (lambdamax-lambdamin)/double(nlambda)
                   ;; Wavelength seperation

;;----------------------------------------------------
;; 2D Interpolation grid in machine coordinatess
;;----------------------------------------------------
nr=100             ;; Number of cells in the radial direction
nw=100             ;; Number of cells in the w direction
rmin=90.           ;; Minimum r value
rmax=230.          ;; Maximum r value
wmin=-100.         ;; Minimum w value
wmax=100.          ;; Maximum w value

;;----------------------------------------------------
;; Define FIDASIM grid in beam coordinates (x,y,z)
;;----------------------------------------------------
...
inputs={comment:comment,shot:shot,time:time,runid:runid,device:strupcase(device),$
        install_dir:install_dir,result_dir:result_dir,cdf_file:cdf_file,profile_dir:profile_dir,$
        emin:emin,emax:emax,pmin:pmin,pmax:pmax,isource:isource,diag:diag,einj:einj,pinj:pinj,$
        ...
        nr:nr,nw:nw,rmin:rmin,rmax:rmax,wmin:wmin,wmax:wmax,$
        nx:nx,ny:ny,nz:nz,xmin:xmin,xmax:xmax,ymin:ymin,ymax:ymax,zmin:zmin,zmax:zmax,$
        origin:origin,alpha:alpha,beta:beta,n_fast:n_fast,n_nbi:n_nbi,n_halo:n_halo,$
        ne_wght:ne_wght,np_wght:np_wght,nphi_wght:nphi_wght,emax_wght:emax_wght,$
