FUNCTION nstx_chords,shot,fida_diag,isource=isource,doplot=doplot
    ...
    fida={nchan:nchans,diag:fida_diag,$
          ulos:double(xloss),vlos:double(yloss),wlos:double(zloss),$
          ulens:double(xheads),vlens:double(yheads),wlens:double(zheads),$
          sigma_pi_ratio:sigma_pis,ra:ras,rd:rds,h:hs,chan_id:chan_ids}

    if keyword_set(doplot) then begin
        npt=11
        for ich=0,fida.nchan-1 do begin
            x=fida.ulens[ich]+(fida.ulos[ich]-fida.ulens[ich])*findgen(npt)/(npt-1.)
            y=fida.vlens[ich]+(fida.vlos[ich]-fida.vlens[ich])*findgen(npt)/(npt-1.)
            oplot,x,y,color=!red
            oplot,fida.ulens,fida.vlens,color=!red,psym=1
        endfor
    endif
